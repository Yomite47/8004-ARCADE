const { PinataSDK } = require("pinata");
const fs = require("fs");
// In Node 20+, File is global.

async function upload() {
    const JWT = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..hclBH5qvpeXK13Ctv4EyPATH5qJRSfY1WUpj3_oKjyE';
    
    const pinata = new PinataSDK({
        pinataJwt: JWT,
        pinataGateway: "gateway.pinata.cloud"
    });

    try {
        console.log("Opening blob for images.car...");
        const blob = await fs.openAsBlob("images.car");
        
        console.log("Blob size:", blob.size);
        
        // Create a File object. 
        const file = new File([blob], "images.car", { type: "application/vnd.ipld.car" });
        
        console.log("Uploading via Pinata SDK v3...");
        // SDK v3 automatically handles large files if implemented correctly
        const upload = await pinata.upload.public.file(file);
        
        console.log("Upload Success!");
        console.log(JSON.stringify(upload, null, 2));
    } catch (error) {
        console.log("Upload Error:");
        console.dir(error, { depth: null });
    }
}
upload();
